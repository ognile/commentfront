...
## North Star
Facebook campaign delivery reaches 100% successful jobs for the current failed/pending backlog, with failures retried only after proxy health, profile health, campaign processor stability, and forensic visibility are proven.

## Success Criteria
- [ ] Current campaign backlog is fully inventoried, with every failed or pending job mapped to one root-cause class and one recovery action.
...
- [ ] Existing failed campaigns are recovered to 100% success or marked impossible with concrete evidence per job: deleted/unreachable post, account challenge, expired media, or platform refusal.
...
- [ ] Production proof captured after review approval: deploy from GitHub, verify healthy proxy and processor, run recovery against the backlog, and read back `/queue`, `/queue/history`, `/analytics/summary`, and `/queue/reliability-audit` showing 100% recovered delivery or documented impossible jobs.
...
- Do not hide failures behind aggregate success rates. 100% means every job has either a successful result or an explicit non-retryable proof.
...
- [ ] Add a read-only backlog audit endpoint/report that merges pending queue, history, retry metadata, result-level failures, and profile health into one ledger.
...
- [ ] Tighten retry-all selection so it includes no-result failed campaigns and pending recovered campaigns, but excludes true auth blockers and impossible posts.
...
- [ ] Execute production recovery in controlled batches, reading back each batch until every job is successful or explicitly impossible.

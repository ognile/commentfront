...
- Runtime behavior may ignore existing session proxy values before migration, but final production state must remove the stale values from session files as well.
- A migration verifier must report `protected_fields_preserved=true` per changed file before and after applying cleanup.
...
| remote browser | `_resolve_remote_active_proxy()` prefers stored session proxy and uses env as fallback | this intentionally encodes stale session proxy authority | remove session proxy input; remote leases resolve active proxy once and log proxy id/source | remote browser source is active proxy store only |
...
- [x] Add read-only diagnostics: active resolver response, proxy store contents, session proxy leak counts, launch-path source labels.
- [x] Refactor proxy model: define `get_active_proxy()` with one contract: active proxy from `ProxyManager` default; env only if no stored proxies exist.
- [x] Remove session proxy authority from launch paths: remote lease, premium safety, reddit helper, reddit reference bootstrap, Facebook login/create/comment flows.
- [x] Remove user-facing session proxy assignment concepts: backend endpoint, proxy manager assigned session authority, UI session proxy filters/status labels that imply session ownership.
- [x] Add migration dry-run: enumerate Facebook and Reddit session files with `proxy`, summarize endpoints, and write no state.
- [x] Add approved migration: backup session files, delete `proxy` fields, preserve cookies/user-agent/viewport/device/tags, and report exact changed files.
...
- `2026-05-22 local tests` -> `python -m pytest backend/tests -q` passed 408 tests; `npm run test` passed 21 frontend tests; `npm run build` passed; `npm run lint` had warnings only.
- `2026-05-22 code search` -> no runtime references remain for `get_system_proxy()`, `session.get_proxy()`, `proxy_source="session"`, `assign-proxy`, `sync-all-to-env-proxy`, or proxy `assigned_sessions`; the only `session.get_proxy()` hit is the negative unit test.
